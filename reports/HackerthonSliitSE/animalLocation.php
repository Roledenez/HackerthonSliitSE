...
$sql = "SELECT `lname`, `maxAnimals` FROM `location` group by `lname` order by `lname` desc";
//$sql = "SELECT `empName`, count(`tid`) as `users` FROM `treatment` group by `empName` order by `tid` desc";
...
<html>
	<head>
		...
		<title>Maximam # of animals per locations</title>
...
$(function () {
    $('#container').highcharts({
        ...
        title: {
            text: 'Maximam # of animals per locations'
        },
        subtitle: {
            text: 'by Adress'
        },
